...
class Race:
    ...
    def Durum(
            self):  # Aracımızın koordinatının herhangi bir dubanın koordinatı ile eşleşip eşleşmediğini kontrol ediyoruz.
        Koordinat = self.canvas.coords(self.idAraba)
    ...
    def Sag(self, event):  # Sağ yön tuşuna basınca X eksninde 1 pixel sağa doğru hareket ettiriyoruz.
        Koordinat = self.canvas.coords(self.idAraba)
        if Koordinat[
            0] != 280:  # Penecereden taşmaması için x eksenindeki koordinatımız 280 olmadığı sürece bu işlemi yapmasını söylüyoruz.
            self.x = 1

    def Sol(self, event):  # Sol yön tuşuna basınca X eksninde 1 pixel sola doğru hareket ettiriyoruz.
        Koordinat = self.canvas.coords(self.idAraba)
        if Koordinat[
            0] != 20:  # Penecereden taşmaması için x eksenindeki koordinatımız 20 olmadığı sürece bu işlemi yapmasını söylüyoruz.
            self.x = -1
...
class Duba:
    ...
    def Ciz(self):
        for ai in range(0, 50):
            canvas.move(self.idDuba[ai], 0,
                        1);  # Dubalarımızı arkaplan ile beraber y ekseninde 1 pixel hareket ettiriyoruz.
        # Böylece arkaplanla beraber hareket ediyormuş gibi gözüküyor.


tk = Tk()
tk.title('Araba Yarışı')  # Oyun Pencere Başlığımız.
...
while 1:  # Sonsuz Döngümüzü Başlatıyoruz.
    if OyunDurumu == 0:  # OyunDurumu değişkeni 0 olduğu sürece gerçekleşecek eylemler.
        canvas.move(idArkaPlan, 0,
                    1);  # Arka Planımızı y ekseninde 1 pixel hareket ettiriyoruz. Sonsuz döngü içerisinde olduğu için bu işlem süreklilik olarak gözükecektir.
    ...
    if Race.Fin()[
        1] > -250:  # Eğer hareketli arkaplanımızın koordinatı -250 yi geçmiş ise oyunun bittiğini ifade ediyoruz.
        OyunDurumu = 2;
